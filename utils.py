...
def fix_title(title):
    ...
    ttable = title.maketrans(\
            'ᴴᴰ🇭🇩🇸🅝🇭🇴🇷🇹🅑🅘🇼🄷🅀🇰🇦🇺🇳🅢🆈🅼🅆🅅🅓🅳🅉🄱ℍ🅀ℚ🅙🅧🅒🅗🅤🅡🅐🄷🅄🄼🅂🄰🄵🄰🅁🅂🄷🄾🅁🅃🆂🅷🅾🆁🆃🄲🄷🄰🄸🄽🄺🄳🅃🄴🄻🄶🄿🅴🅷🆀🅺🆄🅲🅷🅾🆁🅸🅶🅸🅽🅰🅻🅱ⓓⓗⓐⓓⓚⓐⓝⒹ【】🄹🅈',\
            'HDHDSNHORTBIWHQKAUNSYMWVDDZBQHQJXCHURAHUMSAFARSHORTSHORTCHAINKDTELGPEHQKUCHORIGINALBdhadkanD[]JY',\
            '🙏☛☚▫💋🏼‍♂♀👌!.❄🎷🗿👫🔘💥🎙©🆕️☄🚶🚶🤔🥰🎸🕺👈🎼😘/”“🦁⚜️🕉️⏯️🌙"<>[]|💚💖🌸🌻🤪👉💜🐝🍀✔💕💝♥🌹☔🌧️🌩️🌦️🙈™💑®@🎧📝🌷🍁🍂🍃🌼💗👀🤫👑💑🌟🎤💙⚘🙄❤#💗™💘🤹😍💟💞🔥😇🤩😏Ⱥ💃🎈=😔'\
            )
...
def remove_words(title):
    result = title.\
                replace('[HD]','').replace('(HD)','').replace('HD ','').replace(' HD','').replace('ABC -','').\
                ...
                replace('(DUET)','').replace('DUET','').replace('{DUET}','').replace('[CLEAN DUET]','').\
                replace('100%','').replace('(FULL)','').replace('(FULL SONG)','').replace('[FULL]','').replace('FULL','').\
                replace('(100%PURE)','').replace('(CRYSTAL CLEAR)','').replace('[ORIGINAL MUSIC]','').replace('(ORIGINAL TRACK)','').\
                replace('(2 STANZA)','').replace('KARAOKE','').replace('(FEMALE)','').replace('{MINI}','').\
                replace('CLEAR','').replace('COVER','').replace('TRACK','').replace('ORIGINAL MUSIC','').\
                replace('""','').replace(' _ ',' ').replace('_1','').replace('_2','').replace('REMIX','').\
                replace('(AAR PAR)','').replace('(ARTH','').replace('(ARTH)','').replace('(BOMBAY)','').replace('LAWARIS','').\
                replace('(JAANWAR)','').replace('(ARADHANA)','').replace('(LOOTERA)','').replace('(NAU 2 GYARA)','').\
...
def spelling_dict():
    d = {\
            ...
            "Ehsaan Tera Hoga Mujh Par": "Ehsaan Tera Hoga Mujh Par",\
            ...
            "Hume Tumse Pyaar": "Humein Tumse Pyaar Kitna",\
            "Hume Tumse Pyaar Kitna": "Humein Tumse Pyaar Kitna",\
            "Hume Tumse Pyaar Kitna Hume": "Humein Tumse Pyaar Kitna",\
            "Hume Tumse Pyaar Kitna Recreation": "Humein Tumse Pyaar Kitna",\
            ...
            "K​A​U​N Tujhe Tujhe": "Kaun Tujhe",\
            ...
            "Main Koi Aisa Geet Gaaon": "Main Koi Aisa Geet",\
            ...
            "Rimjhim Rimjhim": "Rimjhim Gire Sawan",\
